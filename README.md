...
### 🔧 C 단계: 캐싱 레이어 개선  
**목표**: 반복적인 DB 쿼리를 줄이고 응답 시간 개선

**적용 내용**:
- **Redis 캐시 전략**: 숙소 검색 결과, 상세 정보, 이미지 데이터에 대한 다층 캐싱 구현
  - 검색 필터 기반 결과 캐싱 (검색 조건 조합별 TTL 적용)
  - 숙소 상세 정보 캐싱 (상세 페이지 진입 시 즉시 조회 가능)
  - 이미지 URL 리스트 캐싱 (갤러리 로딩 속도 향상)
  
- **쿼리 최적화**:
  - N+1 문제 해결 (JOIN FETCH / Batch Loading 적용)
  - 복합 조건 검색에 Querydsl 도입으로 동적 쿼리 성능 개선

**빌드 검증**: ✅ Gradle 빌드 통과, 모든 테스트 성공

---

### 🔒 D 단계: 트랜잭션 경계 최적화  
**목표**: 불필요한 트랜잭션 오버헤드 제거 및 성능 향상

**적용 내용**:
- **외부 API 호출 분리**: 
  - Toss 결제 승인 API 호출을 `TransactionTemplate`을 사용해 트랜잭션 **외부**로 분리
  - 결제 승인과 DB 작업을 분리하여 외부 API 지연이 트랜잭션 시간에 영향을 주지 않도록 최적화
  
- **읽기 전용 메서드 최적화**:
  - `@Transactional(readOnly = true)` 적용으로 불필요한 쓰기 락 제거
  - UserService의 `getUserInfo()`, `verifyPassword()` 등 조회 메서드에 readOnly 플래그 적용
  - ReviewService의 `getUserReviews()`, PointHistoryService의 `getHistoryByUser()` 등에도 일괄 적용
  
- **트랜잭션 격리 수준 관리**:
  - 쓰기 작업만 필요한 메서드에 선택적 @Transactional 적용
  - 결과적으로 동시성 처리 성능 향상
...
- **전역 예외 처리**: ApiException, IllegalArgumentException, IllegalStateException에 대한 적절한 HTTP 상태 코드 매핑 ✅
